...
button('Cor da carroceria').props.onFocus()
render()
assert.equal(menu().props.mode, 'paint')
assert.equal(menu().props.waiting, true)
assert.equal(button('Explore o conjunto da bateria').props.disabled, true)
assert.equal(button('Explore a unidade de propulsão').props.disabled, true)
assert.equal(button('Design das rodas').props.disabled, true)
...
button('Explore a unidade de propulsão').props.onClick(); render()
assert.equal(states[0], 'overview', 'Programmatic click must also respect the lock')
menu().props.onClose(); render()
assert.equal(button('Design das rodas').props.disabled, true, 'Remain locked while fading out')
timers.splice(0).forEach(fn => fn()); render()
assert.equal(menu(), null)
assert.equal(button('Design das rodas').props.disabled, false)
assert.equal(states[12], appearance.baseExterior)
button('Design das rodas').props.onFocus(); render()
...
assert.equal(button('Cor da carroceria').props.disabled, true)
button('Design das rodas').props.onClick(); render()
timers.splice(0).forEach(fn => fn()); render()
assert.equal(menu(), null, 'Second click closes pinned mode')
button('Cor da carroceria').props.onFocus(); render()
